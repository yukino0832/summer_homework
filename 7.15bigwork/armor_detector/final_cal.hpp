...
#include "/home/zcy/ZCY/hdu/rm/7.15bigwork/armor_detector/armor.hpp"
#include "/home/zcy/ZCY/hdu/rm/7.15bigwork/armor_detector/classify_number.hpp"
#include "/home/zcy/ZCY/hdu/rm/7.15bigwork/armor_detector/kalman.hpp"
...
public:
    ...
    void calculate(cv::Mat &frame)
    {
        ...
        for (auto &p : pose)
        {
            ...
            distance = pow(p.position[0] - last_pose.position[0], 2) + pow(p.position[1] - last_pose.position[1], 2) + pow(p.position[2] - last_pose.position[2], 2);
            if (!flag)
            {
                flag = true;
